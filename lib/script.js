...
/**
 *
 * @param defines extracted "define"s, referring to exports.extract() for details
 * @param ast which "define"s extracted from
 * @param options {{
 *            id: String/Function,
...
exports.transform = function (defines, ast, options, cb) {
  options = options || {};
  var transformByAlias = (typeof options.alias === 'object') &&
    function(v) {
  ...
  defines.forEach(function(def){
    ...
    var id = idFnc(def.id);
    if (id) {
      if (options.debug && !/-debug$/.test(id)) {
        id += '-debug';
      }
...
exports.generateCode = function(ast, optimized, cb) {
  if (optimized) {
    ast = esmangle.mangle(esmangle.optimize(ast));
  }

  cb(null, escodegen.generate(ast, optimized ? codeStyleUgly : codeStyleBeautify));
};
